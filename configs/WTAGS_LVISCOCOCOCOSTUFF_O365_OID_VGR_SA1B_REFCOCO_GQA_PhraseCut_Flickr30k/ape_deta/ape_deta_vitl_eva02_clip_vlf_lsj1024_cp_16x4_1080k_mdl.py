...
from ...common.data.wtags_lviscocococostuff_o365_oid_vgr_sa1b_refcoco_group_by_image_gqa_phrasecut_flickr30k_panoptic_lsj1024_cp_mdl import (
    dataloader,
)
...
for criterion, num_classes in zip(
    model.model_vision.criterion, [256, 256]
):
...
model.model_vision.criterion[0].weight_dict["loss_class_enc"] = 0.0
for k, v in model.model_vision.criterion[0].weight_dict.items():
    if "_enc" in k:
        model.model_vision.criterion[0].weight_dict.update({k: 0.0})
    if "_bbox" in k or "_giou" in k or "_dice" in k or "_mask" in k:
        model.model_vision.criterion[0].weight_dict.update({k: 0.0})
...
# [xx for x in dataloader.train for xx in x.dataset.names] + [
#     "refcoco-mixed-wtags"
# ]
