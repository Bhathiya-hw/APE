...
do
	echo "=============================================================================================="
	echo ${config_file}
	python3 tools/train_net.py --eval-only --config-file ${config_file} --num-gpus=4 train.output_dir=${output_dir}/${config_file}/"`date +'%Y%m%d_%H%M%S'`" train.init_checkpoint=${init_checkpoint} ${kwargs}
done